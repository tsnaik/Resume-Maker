...
<li>the document now has a baseline direction; this determines the 
	...
	<li>base direction can be set by any of:
		...
		<li>&lt;html output="rtl" or style="direction: rtl;"&gt;</li>
		<li>&lt;body output="rtl" or style="direction: rtl;"&gt;</li>
		</ul></li>
...
<li>language (either CSS "lang", using Autofont, or through initial set-up e.g. $mpdf = new mPDF(\'ar\') )
	...
	NB config_cp.php has been changed as a result; any values of "output" set here are now ineffective</li>
...
<li>improved algorithm for dtermining text direction
	<ul>
	<li>english word blocks are handled in text reversal as one block i.e. output="rtl"<br />
